...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu, virtual_batch_size_ratio):
  ...
  ### New optimizer with gradient accumulation ###
  ### Start ###

  gradient_norm = 1.0
  min_global_norm = 1e-6
  tvars = tf.trainable_variables()
  accum_g = [tf.Variable(tf.zeros_like(tv), trainable=False) for tv in tvars]
  single_grads = optimizer.compute_gradients(loss)
  accum_g_op = [
  ...
  def f_apply():
    ...
    print_op = tf.print("Apply here")
    with tf.control_dependencies([*update_ops, global_step.assign(global_step + 1), print_op]):
  ...
  def f_accumulate():
    step_update_op = global_step.assign(global_step + 1)
    return tf.group(step_update_op)

  with tf.control_dependencies(accum_g_op):
    train_op = tf.cond(
      tf.equal(tf.mod(global_step, virtual_batch_size_ratio), 0),
      f_apply,
      f_accumulate,
    )
